...
<div>
  ...
  <div className="flex item-center gap-5 mt-2">
    ...
    <p className="px-2 sm:px-3 sm:py-1 border bg-slate-50">{item.size}</p>
        ...
        <input  onChange={(e)=>e.target.value ==='' || e.target.value==='0' ? null : updateQuantity(item._id,item.size , Number(e.target.value))} className="border max-w-10  sm:max-w-20 px-1 sm:px-2 py-1" type="number"  min={1} name="" id={item._id}  defaultValue={item.quantity}/>
